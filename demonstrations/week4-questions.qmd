...
$$ This is a random walk with drift $\theta$. What is maximum likelihood estimate for $\theta$? If we use this for forecasting, what is the formula for the $h$-step ahead forecast?

## 2. Drift vs linear trend method

The drift method can be fit using the code `RW(y ~ drift())`. Starting with the following code snippet, compute forecasts for the drift method and the linear trend method for the population of Australia.
